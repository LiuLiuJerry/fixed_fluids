...
*/
	...
	struct bufList {
		float3*			mpos;                              //4 * 3 * 4 = 48
		float3*			mvel;
		float3*			mveleval;
		float3*			mforce;							   //48 + 6 * 4 = 72
		float*			mpress;
		float*			mdensity;		     //存的也是倒数
		uint*			mgcell;
		uint*			mgndx;
		...
		char*			msortbuf;							//72 + 4 = 76  最长的数就是

		uint*			mgrid;									//76 + 5 * 4 = 96
		int*			mgridcnt;    //应该是每个网格的序号
		int*			mgridoff;
		int*			mgridactive;

		//我增加的：
		int*           mphase;  //状态，即水或者冰
		float3*        msurfaceTension;
	};
	...
	struct FluidParams {
		...
		//r2 = psmoothradius * psmoothradius
		...
		//d2 = psimscale * psimscale; rd2 = r2 / d2;
	...
	#define LOG_NUM_BANKS	 4
		...
		//My Add****************************************************************************************
		__global__ void updatePhase(bufList buf, int pnum, float ss);
