...
{ 
	...
	//My add
	CopyToCUDA_My ( (float*) mPos, (float*) mVel, (float*) mVelEval, (float*) mForce, mPressure, mDensity, mClusterCell, mGridNext, (char*) mClr ,mphase, (float *)mSurfaceTension); 
}
...
void FluidSystem::Setup ( bool bStart )
{
	...
	//SetupAddVolume ( m_Vec[PINITMIN], m_Vec[PINITMAX], m_Param[PSPACING], 0.1, m_Param[PNUM] );		//有0.1的偏移量，offs = 0.1											// Create the particles
...
void FluidSystem::AllocateParticles ( int cnt )
{
	...
	mPos = (Vector3DF*)		malloc ( cnt*sizeof(Vector3DF) );  //为变量开辟空间
	if ( srcPos != 0x0 )	{ memcpy ( mPos, srcPos, nump *sizeof(Vector3DF)); free ( srcPos ); }  //若原来的内存不为空，则把原来的内容拷过来
	...
	//My Adds:****************************************************************************************
	int* srcmphase = mphase;
	mphase = (int*)		malloc ( cnt*sizeof(int) );
	if ( srcmphase != 0x0 )	{ memcpy ( mphase, srcmphase, nump *sizeof(int)); free ( srcmphase ); }

	Vector3DF* srcSurfaceTension = mSurfaceTension;
	mSurfaceTension = (Vector3DF*)  malloc ( cnt*sizeof(Vector3DF));
	if ( srcSurfaceTension != 0x0 ) {memcpy ( mSurfaceTension, srcSurfaceTension, nump * sizeof(Vector3DF)); free (srcSurfaceTension); }

	float* srcheat = mheat;
	mheat = (float*)   malloc ( cnt*sizeof(int) );
	if ( srcheat != 0x0 ) { memcpy(  mheat, srcheat, nump * sizeof(float)); free(srcheat); }
	//**********************************************************************************************
...
void FluidSystem::AddEmit ( float spacing )
{
	...
	Vector3DF pos;
	//直接发射成方形
	int x = ceil( pow((float)m_Vec[PEMIT_RATE].y, (float)(1.0/3)) );
	float dx = x*spacing;
	int cnt = x*x;
	int n = 0;
	Vector3DF randPos = m_Vec[PEMIT_POS];
	int initSize = 10;
	randPos.x -= initSize;
	randPos.z -= initSize;
	...
	for ( int y = 0; y < x; y++ ) {	
		for (int xz=0; xz < cnt; xz++ ) {
			...
			p = AddParticle ();
			*(mPos+p) = pos;
			Vector3DF clr ( (pos.x-randPos.x)/dx, (pos.y-randPos.y)/dx, (pos.z-randPos.z)/dx );
			clr *= 0.8;
			clr += 0.2;				
			*(mClr+p) = COLORA( clr.x, clr.y, clr.z, 1); 

			n++;
			if(n == m_Vec[PEMIT_RATE].y)return;
			}
...
void FluidSystem::RunSimulateCUDAFull ()
{
	...
	PERF_PUSH ( "SortCUDA" );
	start.SetSystemTime ();
	PrefixSumCellsCUDA ( 0x0 ); //计算每一个网格的起点相对之前网格的偏移量
	...
	PERF_PUSH ( "PhaseCUDA" );
	start.SetSystemTime ();
	UpdatePhaseCUDA (m_Param[PSIMSCALE]); //判断是液体还是固体
	record ( PTIME_SORT, "Updata phase CUDA", start );
	PERF_POP ();
	
	PERF_PUSH ( "PressureCUDA" );
	start.SetSystemTime ();
	ComputePressureCUDA();//计算压力，分子间作用力
	record ( PTIME_PHASE, "Press CUDA", start );		
	PERF_POP ();
	...
	//PERF_PUSH ( "SurfaceTensionCUDA" );
	//start.SetSystemTime ();
	//ComputeTensionCUDA ();	
	//record ( PTIME_FORCE, "SurfaceTension CUDA", start );
	//PERF_POP ();
...
void FluidSystem::Advance ()
{
	...
	int*        pphase = mphase;
	Vector3DF*  surfaceTension = mSurfaceTension;
	float*      pheat = mheat;
	...
	for ( int n=0; n < NumPoints(); n++ ) {
		...
		//float disTree = 1000;
		//Vector3DF Cp;
		////这里返回两个值：最近的点和最短距离disTree， 这个距离其实是距离的平方
		//bool bcollide = obj.Collide_Tree(*ppos, Cp, disTree);
		//float dd = 0.001;
		//if(bcollide == true ){
		//	Vector3DF va = Cp - *ppos;
		//	Vector3DF vb = *pveleval;
		//	tmp_Count++; 
		//	if(disTree < 1) surfaceTension->Set(0, 0, 0);
		//	if(disTree < dd){
		//		*pphase = 0;
		//		*pclr = COLORA ( 1, 1, 1, 1 );
		//		*pvel = 0;
		...
		//		//贯穿后，速度以一定衰弱比反弹
		//		//Vector3DF v1 = *pvel;
		//		//Vector3DF v2 = va.Normalize();
		//		////速度在法相方向速度的大小
		//		//float nForce = v1.Dot(v2);
		//		//v2 = v2*nForce*(-1);
		//		//Vector3DF dt = v2 + v1;
		//		//Vector3DF newAccel = v2 + dt;
		//		//*pvel = newAccel*(float)0.4;
		//		*ppos = Cp;
		...
		//	}else{
		//		//到达一定的距离时，受到一个垂直于三角面并指向模型，大小和距离成反比的吸引力
		//		Vector3DF vn = Cp-*ppos;
		//		float Dis2 = disTree*disTree;
		//		vn.Normalize();
		//		float attac = 50;//测试参数
		//		Vector3DF attrForce = vn * attac * (float)(1.0/disTree); 
		//		accel.x += attrForce.x; accel.y += attrForce.y; accel.z += attrForce.z;
		//	}
		//}
		...
		Vector3DF prePos = *ppos;
		Vector3DF preVel = *pvel;
		...
		*pvel = vnext;
		...
		Vector3DF tn;
		float t;
		bool intersect = obj.IntersectTest(*ppos, *ppos+vnext, tn, Cp, t);
		if(intersect) {
			*ppos = Cp;
			*pvel = 0;
			...
			*ppos += vnext;						// p(t+1) = p(t) + v(t+1/2) s = 1/2a*t2 + vt;
		}	
...
// Ideal domain size = k*gs/d = k*0.02*2/0.005 = k*8 = {8, 16, 24, 32, 40, 48, ..} //推测是因为数值太小计算不方便
//    (k = number of cells, gs = cell size, d = simulation scale)
void FluidSystem::SetupGridAllocate ( Vector3DF min, Vector3DF max, float sim_scale, float cell_size, float border )
{
	float world_cellsize = cell_size / sim_scale;  //把很小的浮点数放大，变成图形坐标中的一个的cell大小
	
	m_GridMin = min;
	m_GridMax = max;
	m_GridSize = m_GridMax;										//世界坐标喽？
	m_GridSize -= m_GridMin;
	m_GridRes.x = ceil ( m_GridSize.x / world_cellsize );		// Determine grid resolution //世界坐标中有几个cell
	...
	m_GridSize.y = m_GridRes.y * cell_size / sim_scale;			//（m_GridRes.x * cell_size）表示放大的坐标中网格的大小，除以sim_scale是世界坐标中整个网格所占的大小
	m_GridSize.z = m_GridRes.z * cell_size / sim_scale;			//除以一个小数，取整，再乘回来吗？跟精度有关？
...
void FluidSystem::ComputePressureGrid ()
{
	...
	for ( i=0; i < NumPoints(); i++ ) {
		...
		sum = 0.0;
		sumfix = 0.0;
		for(int i = 0; i < obj.m_pModel->nTriangles; i++) obj.findedTri[i] = 0;
		if ( *(mGridCell+i) != GRID_UNDEF ) {
			for (int cell=0; cell < m_GridAdjCnt; cell++) {
				gs =  *(mGridCell+i) - nadj + m_GridAdj[cell];
				//float g0 = obj.ComputPos(*ipos, Vector3DF());
				//g0 += -obj.nadj + obj.m_GridAdj[cell];
				j = m_Grid [gs] ;
				while ( j != GRID_UNDEF ) {
					if ( i==j ) { j = *(mGridNext+j) ; continue; }
					dst = *(mPos + j);
				...
				//修正密度和压强	
				if(gs < 0 || gs > obj.nCell)continue;
				pnode = obj.GridTri[gs];
				while(pnode != NULL){
					////计算是不是在搜索范围内
					//R = obj.closest_Tri(pnode->TriNum, *ipos);
					//dis2 = obj.dist_Point2(R, *ipos);
					//dis2 *= d2;
					//if(dis2 > m_R2){
					//	pnode = pnode->next;
					//	continue;
					...
					int finded = *(obj.findedTri + ntri);
					if(finded == 1){
						pnode = pnode->next;
						continue;
					}else{
						*(obj.findedTri + ntri) = 1;
					}
					//取出三个点
					int *ver = (int*)obj.m_pModel->pTriangles[ntri].VertIdx;
					Vector3DF va = obj.m_pModel->vpVertices[ver[0]];
					Vector3DF vb = obj.m_pModel->vpVertices[ver[1]];
					Vector3DF vc = obj.m_pModel->vpVertices[ver[2]];
					//计算面积
					Vector3DF ab = vb-va;
					Vector3DF ac = vc-va;
					Vector3DF s = ab;
					...
					Vector3DF sample[7];
					for(int s = 0; s < 7; s++)
						sample[s] = va*obj.BarycCoord[s].x + vb*obj.BarycCoord[s].y + vc*obj.BarycCoord[s].z;
					//计算
					//对三角形的每个点，都计算它对粒子p的粘性力,计算积分然后乘以三角形面积
					float tmpsum = 0;
					for(int s = 0; s < 7; s++){
						r2 = obj.dist_Point2(sample[s], *ipos);
						r2 *= d2;
						if(r2 > m_R2)continue;
						c =  m_R2 - r2;
						tmpsum += c * c * c * obj.Weight[s];

					}
					//这里的密度存的是倒数
					float dens = 1.0/obj.WoodDensity;
					tmpsum *= A*(float)m_Param[PRADIUS]*2*dens;
					sumfix += tmpsum;
					pnode = pnode->next;
				}
			}
		...
		sum *= m_Param[PMASS];
		*idensity = (sum + sumfix )* m_Poly6Kern ;	
		*ipress = ( *idensity - m_Param[PRESTDENSITY] ) * m_Param[PINTSTIFF];		
		*idensity = 1.0f / *idensity;//这里存的是密度的倒数，为了下次使用的时候直接乘上密度的倒数
...
void FluidSystem::ComputeForceGrid ()
{
	...
	float r2, r;
	float sumfix;
	Vector3DF fixViscTerm(0, 0, 0);
	Vector3DF fixRepulTerm(0, 0, 0);
	Vector3DF fixPterm(0, 0, 0);
	float r0 = mR;
	float k = 10000000;
			...
			for (int cell=0; cell < m_GridAdjCnt; cell++) {
				...
				while ( j != GRID_UNDEF ) {
					...
					if ( dsq <= m_R2 ) {
						...
						dz = ( ipos->z - jpos.z);
						c = (mR-jdist);
						pterm = d * -0.5f * c * m_SpikyKern * ( *ipress + jpress ) / jdist;//压力部分
						vterm = m_LapKern * visc;//粘性力部分
				...
				//	//得到七个采样点的坐标
					...
					//对三角形的每个点，都计算它对粒子p的粘性力,计算积分然后乘以三角形面积
					fixViscTerm = Vector3DF(0, 0, 0);
					fixRepulTerm = Vector3DF(0, 0, 0);
					fixPterm = Vector3DF(0, 0, 0);
					float forceScale = 0;
					for(int s = 0; s < 7; s++){
						r2 = obj.dist_Point2(sample[s], *ipos);
						r2 *= d2;
						...
						r = sqrt(r2);
						float cc = (mR-r);
						float fdterm = cc * (*idensity) * obj.WoodDensity;
						float fvterm = m_LapKern * 0.95;//粘性力部分

						Vector3DF cwood(0, 0, 0);
						Vector3DF vp = *iveleval;
						//Vector3DF n = obj.m_pModel->vpFacetNorms[ntri];
						//cwood = vp - n*(float)vp.Dot(n);
						//cwood *= 0.2;
						fixViscTerm += (cwood - vp)*fvterm*fdterm * obj.Weight[s];

						//计算压力
						float fpterm = -1* d * cc * m_SpikyKern * (*ipress) / r;//压力部分
						fixPterm += (*ipos - sample[s])*fpterm * fdterm * obj.Weight[s];

						////计算弹力
						//float cf = (mR-r)*(mR-r);
						//float c0 = (mR-r0)*(mR-r0);
						//float Tra = cf*(cf-c0);
						//Tra *= 1.0/(mR*mR*r0*(2*mR-r0));
						//Tra *= k;


						////计算积分然后乘以三角形面积
						//forceScale += obj.Weight[s]*Tra;
					}
					//A *= (float)(1.0/(m_Param[PSPACING]*m_Param[PSPACING]));
					float dens = 1.0/obj.WoodDensity;
					A *= d2 * (float)m_Param[PRADIUS]*2*dens/m_Param[PMASS];
					fixViscTerm *= A;
					//fixRepulTerm = obj.m_pModel->vpFacetNorms[ntri]*forceScale * A;
					fixPterm *= A;

					*iforce += fixViscTerm + fixPterm;
...
void FluidSystem::Draw ( Camera3D& cam, float rad )
{
	...
	switch (1){//(int) m_Param[PDRAWMODE] ) {
	case 0: {
		//glDisable ( GL_TEXTURE_2D );

		glPointSize ( 2 );
		//glEnable ( GL_POINT_SIZE );		////////////////////////这个地方竟然报错 kidding？？？/////////
		glEnable( GL_BLEND ); 
...
void FluidSystem::SetupDefaultParams ()
{
	...
	//  Smoothing Radius (R) =             0.02			m (= 20 mm = ~3/4 inch)  ***********************光滑半径应该是粒子会受到影响的半径范围
	//
	//  Given: D, Pm, N
	...
	//m_Vec [ PEMIT_POS ].Set ( 0, 0, 0 );
	m_Vec [ PEMIT_POS ].Set ( 0, 50, 10 );
	m_Vec [ PEMIT_RATE ].Set ( 100, 2048, 0 );
	m_Vec [ PEMIT_ANG ].Set ( 0, 0, 0 );
	m_Vec [ PEMIT_DANG ].Set ( 0, 0, 0 );
...
//my addition
template<class T>
inline void smoothgradient(const T r, const T R, T &grad) {
	const T q = r/R;
	if (q>1.0 || r==0.0) grad=0.0;
	else {
		const T RR = R*R;
		const T dd = 1-q*q;
		const T alpha = 945.0 / (32.0 * M_PI * RR *R);
		grad = -alpha * dd;
	}
}
template<class T>
T inline smoothkernel(const T r, const T h) {
	const T q = r/h;
	if (q>1.0) return 0.0;
	else {
		return 1.0-q*q;
...
void FluidSystem::ComputeSurfaceTension() {
		int i, j, cnt = 0;
		int nbr;
		float dx, dy, dz, sum, dsq, r;
		float d = m_Param[PSIMSCALE];
		float d2 = d*d;

		float R = m_Param[PSMOOTHRADIUS];
		float space = m_Param[PSPACING];
		float m = m_Param[PMASS];

		Vector3DF*	ipos	= mPos;
		float*		idensity = mDensity;
		Vector3DF*  isurfaceTension = mSurfaceTension;
		Vector3DF*  iforce = mForce;
		float*      jdensity;
		Vector3DF*  jsurfaceTension;
		...
		for(i = 0; i < NumPoints(); i++){
			isurfaceTension->Set(0, 0, 0);
			isurfaceTension++;
		}

		isurfaceTension = mSurfaceTension;
		for(i = 0; i < NumPoints(); i++){
			sum = 0;
			float gradient = 0;
			if ( *(mGridCell+i) != GRID_UNDEF ) {
				for (int cell=0; cell < m_GridAdjCnt; cell++) {
					...
					while ( j != GRID_UNDEF ) {
						...
						dst -= *(mPos + j);//邻居网格中的粒子
						jdensity = mDensity+j;
						jsurfaceTension = mSurfaceTension+j;
						//实际距离的平方
						dsq = d2*(dst.x*dst.x + dst.y*dst.y + dst.z*dst.z);
						//半径范围内的邻居粒子
						if ( dsq <= m_R2 ) {
							...
							r = sqrt(dsq);
							if( r < EPSILON * space ){ j = *(mGridNext+j) ; continue;}
							
							smoothgradient(r, R, gradient);
							//根据密度判断张力大小
							Vector3DF dn = dst * d * (float)((gradient)*(*idensity)*(*jdensity)/r);
							*isurfaceTension += dn*(float)0.5;
							*jsurfaceTension -= dn*(float)0.5;
		...
		for(i = 0; i < NumPoints(); i++){
			...
			Vector3DF surfaceTension(0, 0, 0);
			...
			if ( *(mGridCell+i) != GRID_UNDEF ) {
				for (int cell=0; cell < m_GridAdjCnt; cell++) {
					...
					while ( j != GRID_UNDEF ) {
						...
						if ( dsq <= m_R2 ) {
							r = sqrt(dsq);
							float iweight = smoothkernel(r, R);
							surfaceTension += (*jsurfaceTension)*iweight;
				...
				if(total_weight > 0){
					surfaceTension *= (float)1.0/total_weight;
					*iforce += surfaceTension;
				}
			}
			ipos++;
			isurfaceTension++;
...
void FluidSystem::UpdatePhase(){
	...
	Vector3DF*	ipos	 = mPos;
	int*        iphase   = mphase;
	Vector3DF*  ipvel    = mVel;
	Vector3DF*  ipvelEval= mVelEval;
	DWORD*		ipclr     = mClr;
	float*      iheat    = mheat;
...
//添加吸引力
void FluidSystem::ComputeWoodForce(){
	int i;
	Vector3DF *ipos = mPos;
	Vector3DF *iforce = mForce;
	...
	float dis2, dis;
	float r2, r;
	Vector3DF triAttrTerm;

	//for(i = 0; i < NumPoints(); i++){

	//	RepAdh = obj.computeRepAdh(*ppos, *pveleval, m_Param[PSMOOTHRADIUS],  m_Param[PSIMSCALE], m_Param[PSPACING]);
	//	if(RepAdh.x != 0 && RepAdh.y != 0 && RepAdh.z != 0)
	//		*pforce += RepAdh;

	//	Fric = obj.computFric(*ppos, *pveleval, m_Param[PSMOOTHRADIUS], m_Param[PSIMSCALE], *pdensity, m_LapKern, 0.35, m_Param[PSPACING]);	
	//	if(Fric.x != 0 && Fric.y != 0 && Fric.z != 0)
	//		*pforce += Fric;
			...
			for (int cell=0; cell < m_GridAdjCnt; cell++) {
				...
				while(pnode != NULL){
					...
					for(int s = 0; s < 7; s++){
						...
						float Fun = 1;
						float tmp ;
						//Fun = /*0.007*/1.0/(pow(h, (float)3.25));
						//if(r <= h && 2*r > h){
						//	tmp = -4*r*r/h+6*r-2*h;
						...
						if(tmp < 0) tmp = 0;
						Fun *= tmp;
						triAttrTerm += (sample[s]-(*ipos))*d*Fun*((float)1.0/r) * obj.Weight[s];
