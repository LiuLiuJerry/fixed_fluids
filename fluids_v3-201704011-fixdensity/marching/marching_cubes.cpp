 ...
 * @ref		Efficient implementation of Marching Cubes¡¯ cases with topological guarantees
 */
...
//_____________________________________________________________________________
// Constructor
MarchingCubes::MarchingCubes ( const Vec3r &_origin, real _dx, const Array3r &_phi) : phi(_phi), origin(_origin)
{
	dx = _dx;
	_originalMC = false;
	_ext_data = false;
...
*/
  ...
  printf("%d triangles with %d vertex\n",  trimesh.tri.size(), trimesh.x.size());
}
//_____________________________________________________________________________


void MarchingCubes::swap_trimesh(TriMesh &_trimesh) {
	swap(_trimesh.x,trimesh.x);
	swap(_trimesh.normal,trimesh.normal);
	swap(_trimesh.tri,trimesh.tri);
}
...
//-----------------------------------------------------------------------------
{
  ...
  //trimesh.clear();
  trimesh.x.clear();
  trimesh.normal.clear();
  trimesh.tri.clear();
  for(int i = 0; i < _size_x*_size_y*_size_z; i++)
	  _x_verts[i] = _y_verts[i] = _z_verts[i] = -1;
  /*
...
//-----------------------------------------------------------------------------
{
  ...
  for( int t = 0 ; t < 3*n ; t++ )
  {
	...
	if( t%3 == 2 )
	{
	  trimesh.tri.push_back(Vec3ui(tv[0],tv[1],tv[2]));
	}
  }
}
//_____________________________________________________________________________
...
//-----------------------------------------------------------------------------
{
  ...
  normal[0] = (1-u)*get_x_grad(_i,_j,_k) + u*get_x_grad(_i+1,_j,_k) ;
  normal[1] = (1-u)*get_y_grad(_i,_j,_k) + u*get_y_grad(_i+1,_j,_k) ;
  normal[2] = (1-u)*get_z_grad(_i,_j,_k) + u*get_z_grad(_i+1,_j,_k) ;

  normalize(normal);

  trimesh.x.push_back(vert);
  trimesh.normal.push_back(normal);

  return trimesh.x.size()-1;
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
{
  ...
  normal[0] = (1-u)*get_x_grad(_i,_j,_k) + u*get_x_grad(_i,_j+1,_k) ;
  normal[1] = (1-u)*get_y_grad(_i,_j,_k) + u*get_y_grad(_i,_j+1,_k) ;
  normal[2] = (1-u)*get_z_grad(_i,_j,_k) + u*get_z_grad(_i,_j+1,_k) ;
...
//-----------------------------------------------------------------------------
{
  ...
  normal[0] = (1-u)*get_x_grad(_i,_j,_k) + u*get_x_grad(_i,_j,_k+1) ;
  normal[1] = (1-u)*get_y_grad(_i,_j,_k) + u*get_y_grad(_i,_j,_k+1) ;
  normal[2] = (1-u)*get_z_grad(_i,_j,_k) + u*get_z_grad(_i,_j,_k+1) ;
...
//-----------------------------------------------------------------------------
{
  ...
  vert[0] = vert[1] = vert[2] = normal[0] = normal[1] = normal[2] = 0;

  // Computes the average of the intersection points of the cube
  vid = get_x_vert( _i , _j , _k ) ;
  if( vid != -1 ) { ++u ; const Vec3r &v = trimesh.x[vid] ; const Vec3r &n = trimesh.normal[vid]; vert[0] += v[0] ;  vert[1] += v[1] ;  vert[2] += v[2] ;  normal[0] += n[0]; normal[1] += n[1]; normal[2] += n[2]; }
