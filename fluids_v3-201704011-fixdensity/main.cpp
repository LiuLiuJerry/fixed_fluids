...
void drawScene ( float* viewmat, bool bShade )
{
	...
	if ( iShade <= 1 && bShade ) {		
		...
		//	for (float y=-1000; y <= 1000; y += 100.0 ) {//画四面体 此处是平面
		//		//glVertex3f ( x, 0.0, y );
		...
		//glBegin ( GL_LINES );		
		//for (float n=-100; n <= 100; n += 10.0 ) {//画网格平行于xoy平面的
...
void display () 
{
	...
	// Clear frame buffer
	if ( iShade<=1 ) 	glClearColor( 0.05, 0.05, 0.05, 0.0 );
