...
*/
	...
	// Scalar params
	#define PMODE				0
	#define PNUM				1 //总的粒子数目
	#define PEXAMPLE			2
	#define PSIMSIZE			3
	#define PSIMSCALE			4 //世界坐标中的大小乘以simscale等于实际大小
	#define PGRID_DENSITY		5
	#define PGRIDSIZE			6
	#define PVISC				7
	#define PRESTDENSITY		8
	#define PMASS				9  //单个粒子的质量
	#define PRADIUS				10
	#define PDIST				11 
...
//My add
	...
	//说明：1）结构体变量首地址是其最长基本类型成员的整数倍；
	//      2）结构体每个成员相对于结构体首地址的偏移量都是这个成员大小的整数倍，否则自动填充
	//      3）结构体总大小为结构体最宽基本类型成员大小的整数倍 //这个好像是记录数据的时候用的
	struct Fluid {						// offset - TOTAL: 72 (must be multiple of 12)  //改后TOTAL: 76
		Vector3DF		pos;			// 0
		Vector3DF		vel;			// 12
	...
	private:
		...
		// SPH Kernel functions m_R2=m_Param [PSMOOTHRADIUS] * m_Param[PSMOOTHRADIUS];
