...
void FluidClearCUDA ()
{
	...
	//My add
	cudaCheck ( cudaFree ( fbuf.mphase ),   "Free mphase" );
	cudaCheck( cudaFree(fbuf.msurfaceTension), "Free msurfaceTension");
}
...
{	
	...
	// My add***********************************************************************************************************************
	cudaCheck ( cudaMalloc ( (void**) &fbuf.mphase,     fcuda.szPnts*sizeof(int) ),     "Malloc mphase");
	cudaCheck ( cudaMalloc ( (void**) &fbuf.msurfaceTension,     fcuda.szPnts*sizeof(float)*3 ), "Malloc msurfaceTension");
	//******************************************************************************************************************************
...
void CopyToCUDA_My ( float* pos, float* vel, float* veleval, float* force, float* pressure, float* density, uint* cluster, uint* gnext, char* clr, int* phase, float* surfaceTension )
{
	// Send particle buffers
	int numPoints = fcuda.pnum;
	cudaCheck( cudaMemcpy ( fbuf.mpos,		pos,			numPoints*sizeof(float)*3, cudaMemcpyHostToDevice ), 	"Memcpy mpos ToDev" );	
	...
	cudaCheck( cudaMemcpy ( fbuf.mclr,		clr,			numPoints*sizeof(uint), cudaMemcpyHostToDevice ), 		"Memcpy mclr ToDev"  );
	//My add
	cudaCheck( cudaMemcpy ( fbuf.mphase,	phase,			numPoints*sizeof(int), cudaMemcpyHostToDevice ), 		"Memcpy mphase ToDev"  );
	cudaCheck( cudaMemcpy ( fbuf.msurfaceTension, surfaceTension, numPoints*sizeof(float)*3, cudaMemcpyHostToDevice),"Memcpy msurface ToDev");
...
void CopyFromCUDA_My ( float* pos, float* vel, float* veleval, float* force, float* pressure, float* density, uint* cluster, uint* gnext, char* clr ,int mphase)
{
	// Return particle buffers
	int numPoints = fcuda.pnum;
	if ( pos != 0x0 ) cudaCheck( cudaMemcpy ( pos,		fbuf.mpos,			numPoints*sizeof(float)*3, cudaMemcpyDeviceToHost ),	"Memcpy mpos FromDev"  );
...
void preallocBlockSums(unsigned int maxNumElements)
{
	...
	//大规模规约的时候的内存分配，先用好几个block开辟第一次规约的内存，再用好几个block进行第二次规约
    do {       
        unsigned int numBlocks = max(1, (int)ceil((float)numElts / (2.f * blockSize)));
        if (numBlocks > 1) 
...
void prescanArrayRecursiveInt (int *outArray, const int *inArray, int numElements, int level)
{
    ...
    unsigned int numEltsPerBlock = numThreads * 2;//每个线程两个元素

    // if this is a non-power-of-2 array, the last block will be non-full
    // compute the smallest power of 2 able to compute its scan. //能把所有数据包含的2的最小次方
    unsigned int numEltsLastBlock = numElements - (numBlocks-1) * numEltsPerBlock;
    unsigned int numThreadsLastBlock = max(1, numEltsLastBlock / 2);
    unsigned int np2LastBlock = 0;
    unsigned int sharedMemLastBlock = 0;
...
//My add
void UpdatePhaseCUDA (float ss )
{
    updatePhase<<< fcuda.numBlocks, fcuda.numThreads>>>(fbuf, fcuda.pnum,ss);
}

void ComputeTensionCUDA ()
{
	computeTension<<<fcuda.numBlocks, fcuda.numThreads>>>(fbuf, fcuda.pnum);
	cudaError_t error = cudaGetLastError();
	if (error != cudaSuccess) {
		fprintf ( stderr, "CUDA ERROR: ComputeTensionCUDA: %s\n", cudaGetErrorString(error) );
	}    
	cudaThreadSynchronize ();
}